Feature: Loan Application Submission and Approval
  ...
  Scenario: Co-Borrower FICO Impact on Eligibility
    ...
    And the interest rate should be calculated based on the "Conventional" program
    # Then the application should be denied - incorrectly calculated
  ...
  Scenario: Loan Amount at Program Boundaries
    ...
    And the property type is "Single Family"
    And the LTV is "80"% on a loan of $"418000"
    And the loan amount is $"418000"
    And the loan type is "Fixed"
    And the loan period is "30 years"
    When the borrower submits the loan application
    Then the application should be approved
  ...
  Scenario: Invalid Loan Type and Period Combination
    ...
    And the property type is "Condominium"
    And the LTV is "75"% on a loan of $"300000"
